...
决策级知识是专家的决策经验，得到最终的对策信息。
# 第六章 地图设计的智能化方法
## 地图设计中的地图制图知识
> 地图制图知识就是地图设计与编绘过程中，专家经验和各种地图制图规范的总称。
涉及对地图图形特征和空间关系的描述
- 过程性知识：地图设计中如何调用各种操作或模块
- 描述性知识：用不同的参数值定义一个地图制图对象的谓词(事实)
- 决策性知识：选择不同地图设计方案的知识，判断方案优劣
- 分类标准
    - 知识的应用范围
    - 有无空间特性
    - 知识的来源
...
- 对数据进行概念化描述
    - 问题的领域模型
        - 空间参考数据、地理实体
        - 属性信息集合
    - 概念索引：依据每个数据项在问题领域中的含义，来表达它们的意义。帮助系统理解数据的含义、不同数据成分之间的关系
        - 系统变量
        - 特征变量
- 属性表的数据可以和几何数据分开存储
- 地理语义数据和地理几何数据的对应关系可以通过附加信息来建立。地理数据的描述（目标标识码、属性表名称），或是映射关系
## 基于先验知识的地图色彩设计
- 设色对象：图廓、点、线、面
- 地图类型
- 地图特征
...
- 20分的题
    - 广度遍历过程
    - 注记配置的数学规划法（最优化法）
    - 框架表示法与产生式规则的差异，使用框架表示地图投影选择的知识，并说明推理过程